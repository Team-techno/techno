...
async def lego(event):
 ...
 if event.sender_id != OWNER_ID and not quew:
  await event.reply('`ɢɪᴠᴇ sᴏᴍᴇ ᴛᴇxᴛ ᴛᴏ ᴄʀᴇᴀᴛᴇ ʟʟᴏo !`\n`Example /logo <Techno>`')
  return
 ...
 try:
    ...
    fname = "fallen.png"
    img.save(fname, "png")
    await telethn.send_file(event.chat_id, file=fname, caption = f"━━━━━━━━━━━━━━━━\nʟᴏɢᴏ sᴜᴄᴄᴇssꜰᴜʟʟʏ ɢᴇɴᴇʀᴀᴛᴇᴅ ʙʏ​  ‌ٖٖٖٖٖٖٜٖٖٖ[Techno Robot](https://t.me/Techno_ro_bot)\n━━━━━━━━━━━━━━━━")         
    await pesan.delete()
    if os.path.exists(fname):
            os.remove(fname)
 except Exception as e:
    await event.reply(f'Error, Report @Technobotsupport')

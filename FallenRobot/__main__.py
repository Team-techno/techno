...
buttons = [
    [
        InlineKeyboardButton(
            text="Add me to your group", url="t.me/FallenXRobot?startgroup=true"),
    ],
    [
        InlineKeyboardButton(
...
def start(update: Update, context: CallbackContext):
    ...
    if update.effective_chat.type == "private":
        ...
        update.effective_message.reply_text(
            "TECHNO ROBOT IS ALIVE !\n<b>I DIDN'T SLEEP:</b> <code>{}</code>".format(
                uptime
            ),
...
def get_help(update: Update, context: CallbackContext):
    ...
    if chat.type != chat.PRIVATE:
        if len(args) >= 2 and any(args[1].lower() == x for x in HELPABLE):
            ...
            return
        update.effective_message.reply_text(
            "» ᴀᴠᴀɪʟᴀʙʟᴇ ᴄᴏᴍᴍᴀɴᴅs ᴏғ *Techno Robot*.",
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
...
def main():
    ...
    if SUPPORT_CHAT is not None and isinstance(SUPPORT_CHAT, str):
        try:
            dispatcher.bot.sendMessage(f"@{SUPPORT_CHAT}", "Techno Robot is alive sir/mam")
        except Unauthorized:
            LOGGER.warning(
